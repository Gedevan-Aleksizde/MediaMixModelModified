...
bhill <- function(x, b, k, s) b * MediaMixModelModified::hill(x, k, s)
...
roas <- ROAS(res, df_out, L = 13)
mroas <- mROAS(res, df_out, L = 13)
```
...
ggplot(roas, aes(x = ROAS, group = media)) + geom_histogram(bins = 30) +
  facet_wrap(~media)
ggplot(mroas, aes(x = ROAS, group = media)) + geom_histogram(bins = 30) +
  facet_wrap(~media)
```
